...
**分支互斥（第三轮补充，第四轮修正）**：一个分支 B 只在一个条件下被拦——改写某行会破坏
**未达标兄弟**（`actual <= target + tolerance`）的命中，即该行当前命中兄弟谓词、而 `set` 之后不再
命中。**超目标的兄弟不拦**：它的富余行必须能被转化，否则互补谓词（二值列 `A` 0.3 / `B` 0.7 覆盖
全部行）会因为「零候选」而冻结在训练分布上；写的是兄弟谓词读不到的列也不拦，因此不同列的两个
分支能叠加在同一批行上（见 §9.1/#2 与 §9.5）。
...
| 2 | 修复循环里，只有「改写会破坏**未达标**兄弟命中」的行才受保护；超目标兄弟的富余行可被转化，写不同列的两个分支可叠加在同一批行上 | `set` 只能让行**命中**，未达标的兄弟补不回来；而「命中任一分支即全部锁定」会让互补谓词（二值列）零候选、直接冻结（§9.5） |
...
### 9.5 第四轮：`reserved` 过宽（复审意见 4017901726）

复审指出第三轮的 `reserved`（命中任一分支的行都不许别人改写）会把互补谓词锁死。复现成立，
两种形态都复现了：

- 二值列 `A`/`C`（训练 24.5%/75.5%），分支 `status=='A'` 0.3 与 `status=='C'` 0.7 覆盖**所有**行，
  于是 `reserved` = 全表、候选为空：修复前 `a Fail 0.245`、`c Fail 0.755`，0 轮 0 改写（冻结）。
- 两个不同列的分支（`status=='A'` 90% 与 `qty==99` 50%）本可叠加在同一批行上，修复前第二个分支
  只能吃到第一个分支没用的 20 行 → `Warn 0.100`。

修正后的规则（`generate` 的 `protected` + `destroys_protected_coverage`）：
第 b 个分支在行 r 上的候选资格 = `r` 不命中 b 的谓词 ∧ 对每个 `c != b`，不存在
「`c` 未达标（`actual <= target + tolerance`，按**本轮开始**度量）且 `r` 命中 c 且在 r 上施加 b 的
赋值后不再命中 c」。修复后同一批 fixture：二值互补 `a Pass 0.300`（1 轮 11 行）/`c Pass 0.700`；
叠加场景 `a Pass 0.900`（1 轮 131 行）与 `qty==99 Pass 0.500`（1 轮 100 行）。两条新测试
