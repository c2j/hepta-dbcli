...
- **F17（issue #108 / #109 回填，2026-09-21）**：① **#108**：WP2 的 PK 区间探针（§6.3 的 range 路径）在「路由键为空」即无键表上也执行，并把键名兜底成 `id`——PostgreSQL 系引擎里 `SELECT MIN(id) ...` 报 `column "id" does not exist` 后，快照事务（`BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY`）进入 aborted，后续每条语句（先是 checksum，再是全部）都变成 `SQLSTATE 25P02`，用户看到的报错因此指向 checksum SQL 而非探针。修复三点：(a) `range_probe_key` 只在存在单列键且其声明类型可能是整数时探针（varchar/date/json 直接走 `MOD(rowHash, N)`，零查询）；(b) 探针错误分型——`Domain`（空表/全 NULL/非整数/区间不相交）继续回退 MOD，`Db`（引擎拒绝语句）上抛为一条带键名与替代策略的报错，不再静默降级；(c) 探针与区间谓词/校验一律使用该侧配对后的物理键名并加标识符引用（与 §6.2.2 的 `key_range` 对齐）。实测 openGauss 5.0.0：无键表 snapshot 修复前逐字复现 25P02、修复后出 keyless multiset 报告；`MIN(json)` 失效路径在 snapshot 与 `--consistency none` 下均报 `bucketdiff key-domain probe failed on key 'payload'`。回归：DuckDB 无 `id` 列的 keyless 表经 MCP `delta_diff`（默认 snapshot）全链路用例，已用临时回退验证其为真 Red。② **#109**：新增 `--exclude-columns`（CLI，逗号分隔）与 MCP `exclude_columns`，在自动发现/`--columns` 之后做减法；未知名、与 `--columns` 重叠、全部排除均为配置错误，键列排除保留行身份并写入 warning。③ 复现过程中另发现独立缺陷：`list_tables` 的 `obj_description` 在 SQL_ASCII 实例上触发 22021，使 bucketdiff 桶数估算（`estimate_rows`）失败，已单开 issue #111，本轮未修。
